...
const AGENT_PERMISSIONS: AgentPermissionConfig = {
  build: [
    ...
    // Account for additive rules: appending after the catch-all allow makes
    // them win (evaluate() matches in reverse order).
    { tool: 'secret_manager', resource: '*', effect: 'ask', scope: 'workspace' },
  ],
  plan: [
    ...
    { tool: 'inspect', resource: '*', effect: 'allow', scope: 'workspace' },
    ...
    { tool: 'run_command', resource: '*', effect: 'allow', scope: 'workspace' },
    { tool: 'secret_manager', resource: '*', effect: 'ask', scope: 'workspace' },
  ],
  explore: [
    { tool: 'read_file', resource: '*', effect: 'allow', scope: 'workspace' },
